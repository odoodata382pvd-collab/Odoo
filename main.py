# main.py - bản cập nhật: sửa đúng "tồn kho chi tiết (Có hàng)" và format trả về theo yêu cầu
import os
import io
import logging
import pandas as pd
...
# Xử lý ODOO_URL: đảm bảo dạng base (không kèm /odoo)
...
# --- Helper: Escape Markdown V2 (nếu cần) ---
...
def get_stock_data():
    ...
    try:
        ...
        if len(location_ids) < 3:
            error_message = f"không tìm thấy đủ 3 kho cần thiết: {list(location_ids.keys())}"
...
# --- 4. Hàm xử lý Tra Cứu Sản Phẩm ---
async def handle_product_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Tra cứu nhanh tồn kho theo Mã sản phẩm (default_code).
    Summary dùng 'qty_available' (Hiện có).
    Detail (cần chuẩn xác): dùng 'quantity' (Có hàng) từ stock.quant, cộng dồn theo location_id.
    """
    ...
    try:
        # Lấy các location cần cho summary
        location_ids = find_required_location_ids(models, uid, ODOO_DB, ODOO_PASSWORD)
        ...
        # --- Summary: qty_available theo từng kho (Hiện có) ---
        ...
        # --- Detail: LẤY TỒN KHO CHI TIẾT (Có hàng) từ stock.quant ---
        # Lấy tất cả stock.quant có quantity > 0 cho sản phẩm
        ...
        # Lấy thông tin tên location (complete_name nếu có) cho tất cả ID xuất hiện
        ...
        # ====== FIX CHÍNH: cộng dồn chính xác theo ID kho dựa trên field 'quantity' ======
        # Sử dụng float để cộng dồn, sau đó hiển thị bằng int() (cắt phần thập phân)
        stock_by_loc_id = {}
        for q in quant_data_all:
            ...
            # Chỉ cộng các entry có qty > 0 (điều kiện search_read đã đảm bảo)
            if qty <= 0:
                continue
            # Cộng dồn theo id kho
            stock_by_loc_id[loc_id] = stock_by_loc_id.get(loc_id, 0.0) + qty

        # Chuyển sang tên kho và làm int (không dùng round để tránh sai lệch)
        all_stock_details = {}
        for loc_id, qty in stock_by_loc_id.items():
            ...
            qty_int = int(qty)  # cắt phần thập phân để khớp cột Có hàng
            if qty_int > 0:
                all_stock_details[display_name] = qty_int

        # --- Tính đề xuất (giữ nguyên logic cũ) ---
        ...
        # --- Format tin nhắn theo đúng thứ tự bạn yêu cầu ---
        # Dòng 1: "<Mã> <Tên sản phẩm>"
        header_line = f"{product_code} {product_name}"

        # Các dòng summary (hn, hcm, hn transit, recommendation)
        summary_lines = [
            f"tồn kho hn: {int(hn_stock_qty)}",
            f"tồn kho hcm: {int(hcm_stock_qty)}",
            f"tồn kho nhập hà nội: {int(hn_transit_qty)}",
            recommendation_text.replace('`', '')  # hiện dưới dạng plain text (bỏ dấu markdown)
        ]

        # Sắp xếp tồn kho chi tiết: ưu tiên các kho trong PRIORITY_LOCATIONS (so sánh theo code trong tên)
        priority_items = []
        other_items = []
        used_names = set()

        # First, try to find priority matches (match substring)
        ...
        # Then add others sorted by name (stable)
        for name, qty in sorted(all_stock_details.items()):
            if name not in used_names:
                other_items.append((name, qty))
                used_names.add(name)

        # Build detail lines exactly like user requested (name: qty)
        detail_lines = []
        for name, qty in priority_items + other_items:
            detail_lines.append(f"{name}: {qty}")

        if not detail_lines:
            detail_content = "không có tồn kho chi tiết lớn hơn 0."
        else:
            detail_content = "\n".join(detail_lines)

        # Final message: match your requested format
...
"""
        ...
        # Gửi message plain text (không dùng Markdown) để giữ nguyên format
...
async def excel_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("đang xử lý dữ liệu và tạo báo cáo excel. vui lòng chờ...")
